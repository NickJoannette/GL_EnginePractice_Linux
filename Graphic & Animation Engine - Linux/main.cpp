...
bool useBlinnLightingOnTextureCubes = true;
...
Camera camera(glm::vec3(0,1.75,1.5));
Model m("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Models/desk.obj", true);
Model m2("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Models/deskLamp.obj", true);
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS)
		useBlinnLightingOnTextureCubes = !useBlinnLightingOnTextureCubes;

    if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS)
		restrictToRoom = !restrictToRoom;
	...
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
        ...
        if (camera.Position.y >= 3.4) camera.Position.y = 3.4;
...
#pragma endregion
	...
	Shader lightSourceShaderProgram("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/lightSourceShader.vs",
	"/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/lightSourceShader.fs");
	Shader cubeShaderProgram("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/texturedCubeShader.vs", "/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/texturedCubeShader.fs");
	Shader backgroundShaderProgram("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/backgroundTextureShader.vs", "/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/backgroundTextureShader.fs");
	Shader surfaceShader("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/basicShader.vs", "/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/basicShader.fs");
	Shader modelLoadingShader("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/model_loading.vs", "/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Shaders/model_loading.fs");
...
#pragma region background buffer objects
	...
	unsigned char *bgData = stbi_load("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Textures/cursor1.png", &bgWidth, &bgHeight, &bgNrChan, STBI_rgb_alpha);
	if (bgData)
	{
...
#pragma region textures
	...
	stbi_set_flip_vertically_on_load(true);
	unsigned char *mountainData = stbi_load("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Textures/brickTex.jpg", &mountainWidth, &mountainHeight, &mountainNrChan, STBI_rgb_alpha);
	if (mountainData)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB, mountainWidth, mountainHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, mountainData);
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	else
	{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	stbi_set_flip_vertically_on_load(true);
	unsigned char * data = stbi_load("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Textures/brickTex.jpg", &width, &height, &nrChan, STBI_rgb_alpha);
	if (data)
	{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);
    int normalWidth, normalHeight, normalNrChan;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	stbi_set_flip_vertically_on_load(true);
	unsigned char * normalMapData = stbi_load("/home/nicholasjoannette/Desktop/OpenGL Projects/GLEW Test/Textures/normalMap.jpg", &normalWidth, &normalHeight, &normalNrChan, STBI_rgb_alpha);
	if (normalMapData)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, normalWidth, normalHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, normalMapData);
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	else
	{
...
/*
		...
		glm::vec3 lightSourcePosition = glm::vec3(6.0, 5.0, 0);
...
#pragma region draw utility cube
		...
		cubeShaderProgram.setBool("blinn", useBlinnLightingOnTextureCubes);
		cubeShaderProgram.setBool("gammaCorrection", useGammaCorrection);
...
#pragma region cube drawing
	    ...
	    for (int i = 0; i < 5; ++i) for (int j = 0; j < 5; j ++){
                cubeShaderProgram.setMat4("model", glm::translate(cubeModel, glm::vec3(i, 4, j)));
				glDrawArrays(GL_TRIANGLES, 0, 36);
		}

        for (int i = 0; i < 5; ++i) for (int j = 0; j < 5; j ++){
                cubeShaderProgram.setMat4("model", glm::translate(cubeModel, glm::vec3(-1, i, j)));
				glDrawArrays(GL_TRIANGLES, 0, 36);
		}
       for (int i = 0; i < 5; ++i) for (int j = 0; j < 5; j ++){
                cubeShaderProgram.setMat4("model", glm::translate(cubeModel, glm::vec3(5, i, j)));
				glDrawArrays(GL_TRIANGLES, 0, 36);
		}


        for (int i = 0; i < 5; ++i) for (int j = 0; j < 5; j ++){
                cubeShaderProgram.setMat4("model", glm::translate(cubeModel, glm::vec3(j, i, -1)));
				glDrawArrays(GL_TRIANGLES, 0, 36);
		}
       for (int i = 0; i < 5; ++i) for (int j = 0; j < 5; j ++){
                cubeShaderProgram.setMat4("model", glm::translate(cubeModel, glm::vec3(j, i, 5)));
				glDrawArrays(GL_TRIANGLES, 0, 36);
		}
